  ...
  return (
    <>
      <div className='user-box'>
        ...
        <div className='up'>
          <h1>Win %</h1>
          {winPercentage ? <h2>{winPercentage}</h2> : <h2>Play More Games</h2>}
        </div>
        <div className='up'>
        ...
        <table>
          <tbody>
            {allObjects.map((stat, index) => {
              let barWidth = stat === 0 ? '4' : (Math.floor((stat / gamesPlayed) * 80) + 8).toString();
              let barColor = index === 1 ? 'green' : 'rgb(107,107,109';
              console.log(stat);
